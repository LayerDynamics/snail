...
- **Built, tested, committed (9 workspace members, ~151 tests, all clippy `-D warnings` + fmt clean):**
  ...
  - `services/snail_server` — the **composition root**: `Server` wires auth + shared store + `Mta`/filter + access + STARTTLS certs + outbound relay + firewall; async TCP listeners for submission/POP3/IMAP/**inbound-MX** with ctrl-C shutdown, each a bounded `accept_loop` with a **per-listener concurrency cap** (`ConcurrencyLimits`; a permit acquired before `accept`, so the public port can't exhaust tasks/sockets nor starve the authenticated listeners); `snail-server` binary. **All four listeners upgrade plaintext→TLS in-session via the shared `MaybeTlsStream` enum + `accept_tls` helper** (submission/inbound `STARTTLS`, POP3 `STLS`, IMAP `STARTTLS`). Verified e2e (submit→deliver→retrieve) in-process and over TCP (m15). **m16 wired the full internet MTA**: `outbound.rs` (SMTP client `relay_to` + MX-resolving `relay`, with opportunistic STARTTLS upgrade via the `RelayStream` enum), `spool.rs` (durable qmail-style retry queue surviving restart), `worker.rs` (background relay worker with exponential backoff + bounce), `serve_inbound` (no-auth port-25 receiver, no open relay, STARTTLS upgrade), and a firewall-gated accept. A two-server submission→relay→inbound e2e proves it.
...
- **Outbound relay & inbound MX are now wired into the running server (m16):** authenticated submission spools remote recipients per-domain to a durable on-disk queue (`<data_dir>/spool`); a background worker resolves MX via `network` and relays them, **opportunistically upgrading each delivery to STARTTLS when the exchange advertises it** (encrypt-if-offered, like Postfix `smtp_tls_security_level = may`; a failed handshake defers and retries rather than downgrading to cleartext), retrying with exponential backoff and bouncing on exhaustion. The inbound MX listener (default `127.0.0.1:2525`; `:25` in production) accepts external mail to local recipients only — refusing non-local `RCPT` so Snail is never an open relay — advertises and performs STARTTLS, is rate-limited by `security::Firewall` at accept time, and bounds each connection's input (a per-line cap, a per-message `DATA` size cap, and a per-transaction recipient cap — refused with `500`/`552`/`452`, so an unauthenticated peer cannot OOM the process or fan out delivery without bound). **Remaining hardening (future):** outbound STARTTLS is opportunistic (encrypts against passive eavesdroppers but does not authenticate the peer certificate) — **authenticated outbound TLS via DANE / MTA-STS** is not yet implemented; the exchange→IP hop uses the OS resolver (not hickory); SPF/DKIM verification on inbound and greylisting are not yet implemented.
...
- **Client access protocols — `crates/access/`** (implemented): how mail clients talk to the server — `imap.rs`, `pop.rs`, `msa.rs` (mail submission agent), `dovecot.rs` (Maildir++ mapping), `web.rs`, coordinated by `manager.rs` (`AccessManager`). The IMAP and POP sessions are TLS-aware (`ImapSession::with_tls`/`Pop3Session::with_tls`): they advertise `STARTTLS`/`STLS` (+ IMAP `LOGINDISABLED`) and refuse `LOGIN`/`USER`+`PASS` until the connection is encrypted; the socket loops in `serve.rs` perform the handshake. SMTP submission STARTTLS is loop-driven in `serve_submission`, mirroring the inbound receiver.
...
- `snail_server/` — the composition root (implemented): `lib.rs` exposes `Server` (wires auth + shared store + `Mta`/filter + access + STARTTLS certs + relay context + firewall) and `install_crypto_provider()`; `serve.rs` has the async listeners (`run`/`serve_submission`/`serve_pop`/`serve_imap`/`serve_inbound`/`serve_inbound_firewalled`) plus the shared `MaybeTlsStream` plaintext→TLS upgrade enum and `accept_tls` helper used by all four protocol loops; `outbound.rs` is the SMTP client (`relay_to` + MX-resolving `relay`) with opportunistic STARTTLS (the `RelayStream` plaintext→TLS upgrade enum); `spool.rs` is the durable `OutboundSpool` retry queue; `worker.rs` is the background `relay_due`/`spawn_relay_worker`; `main.rs` is the `snail-server` binary (rustls provider, telemetry, certs, spool, system resolver, users, listeners, ctrl-C). The `tests/relay_e2e.rs` integration test drives submission→relay→inbound across two servers.
