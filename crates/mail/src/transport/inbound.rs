...
impl InboundCollector {
    ...
    /// wire, as **raw bytes** (never lossily UTF-8-decoded), so 8-bit/binary
    /// content and DKIM-signed headers are accumulated verbatim. Returns `true`
    /// only when the genuine end-of-data marker `<CRLF>.<CRLF>` is seen.
    ///
    ...
    pub fn push_line(&mut self, line: &[u8]) -> bool {
        let this_crlf = line.ends_with(b"\r\n");
        let content = line
            .strip_suffix(b"\r\n")
        ...
        if self.prev_crlf && this_crlf && content == b"." {
            self.finished = true;
            return true;
        }

        let unstuffed = content.strip_prefix(b".").unwrap_or(content);
...
mod tests {
    ...
    fn collects_body_until_dot_then_parses() {
        ...
        assert!(!c.push_line(b"Subject: Hi\r\n"));
        assert!(!c.push_line(b"\r\n"));
        assert!(!c.push_line(b"hello world\r\n"));
    ...
    fn dot_unstuffs_leading_dots() {
        let mut c = InboundCollector::new();
        c.push_line(b"Subject: x\r\n");
        c.push_line(b"\r\n");
        c.push_line(b"..hidden\r\n"); // sender stuffed ".hidden" -> "..hidden"
        c.push_line(b".\r\n");
        let msg = c.into_message(envelope()).unwrap();
    ...
    #[test]
    fn preserves_non_utf8_body_bytes_verbatim() {
        // 8-bit / legacy-charset content must survive byte-for-byte: a lossy UTF-8
        // round-trip would replace 0xFF/0xFE with U+FFFD and break DKIM.
        let mut c = InboundCollector::new();
        c.push_line(b"Subject: =?ISO-8859-1?Q?caf=E9?=\r\n");
        c.push_line(b"\r\n");
        c.push_line(b"caf\xe9 \xff\xfe binary\r\n"); // invalid UTF-8 on purpose
        assert!(c.push_line(b".\r\n"));
        let msg = c.into_message(envelope()).unwrap();
        assert_eq!(msg.body, b"caf\xe9 \xff\xfe binary\r\n");
        // And the full serialization round-trips the raw bytes unchanged.
        assert!(msg.to_bytes().ends_with(b"caf\xe9 \xff\xfe binary\r\n"));
    }
    ...
    fn bare_lf_dot_is_not_a_terminator() {
        ...
        let mut c = InboundCollector::new();
        assert!(!c.push_line(b"first\r\n"));
        assert!(!c.push_line(b".\n")); // bare-LF dot — NOT a terminator
        assert!(!c.push_line(b"still body\r\n"));
    ...
    fn smuggled_commands_stay_in_the_body() {
        ...
        let mut c = InboundCollector::new();
        c.push_line(b"Subject: legit\r\n");
        c.push_line(b"\r\n"); // end of headers
        c.push_line(b"body start\r\n");
        c.push_line(b".\n"); // smuggling attempt
        c.push_line(b"MAIL FROM:<spoofed@example.com>\r\n");
        c.push_line(b"RCPT TO:<victim@example.com>\r\n");
        c.push_line(b"Injected\r\n");
